...
class ModernInterface(QMainWindow):
    def __init__(self, menu_config):
        ...
        for i, item in enumerate(self.menu_config):
            ...
            button.setStyleSheet(
                "QToolButton { background-color: #2C3E50; color: white; border-radius: 10px; padding: 5px; }"
    ...
    def change_screen(self, index):
        ...
        for i, button in enumerate(self.menu_buttons):
            if i == index:
                button.setStyleSheet(
                    "QToolButton { background-color: #16A085; color: white; border-radius: 10px; padding: 5px; }"
                    "QToolButton:hover { background-color: #1ABC9C; }"
                    "QToolButton:pressed { background-color: #1ABC9C; }"
                )
            else:
                button.setStyleSheet(
